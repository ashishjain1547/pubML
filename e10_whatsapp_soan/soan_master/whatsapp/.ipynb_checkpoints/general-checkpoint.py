...
def plot_active_hours(df, color='#ffdfba', savefig=False, dpi=100, user='All'):
    ...
    # Save or show figure    
    if savefig:
        plt.savefig('active_hours.png', dpi = dpi)
    else:
        plt.show()
...
def plot_active_days(df, savefig=False, dpi=100, user='All'):
    ...
    count = df['Day_of_Week'].value_counts().sort_index().values
    ...
    # Save or show figure    
    if savefig:
        plt.savefig('active_days.png', dpi = dpi)
    else:
        plt.show()
...
def plot_day_spider(df, colors=None, savefig=False, dpi=100):
    ...
    for index, user in enumerate(df.User.unique()):
        ...
        ax.plot(angles, values, linewidth=2, linestyle='solid', zorder=index, color=colors[index % len(colors)], alpha=0.8)
        ax.fill(angles, values, colors[index % len(colors)], alpha=0.1, zorder=0)

        if max(values) > max_val: max_val = max(values) # To align ytick labels
            
        legend_elements.append(Line2D([0], [0], color=colors[index % len(colors)], lw=4, label=user))
...
def plot_messages(df, colors=None, trendline=False, savefig=False, dpi=100):
    ...
    for i, user in enumerate(users):
        ax.plot(users[user].Date, users[user].Message_Raw, linewidth=3, color=colors[i % len(colors)])
        legend_elements.append(Line2D([0], [0], color=colors[i % len(colors)], lw=4, label=user))
...
def print_stats(df, love=False):
    ...
    * How often user said "I love you"
    ...
    if love:   
        ...
        print_title('How often user said "I love you"')
        
        for user in df.User.unique():
            print('{0: <30}'.format(user + ':') + str(sum(df[df.User == user].Love)))
...
def calendar_plot(data, year=None, how='count', column = 'User', savefig=False, dpi=100):
    ...
    * Set date as index
    ...
    ax.set_xticks([daily.loc[datetime.date(year, i + 1, 15),:].week for i in monthticks])
